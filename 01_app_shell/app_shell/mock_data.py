...
def build_workspace_from_fixture() -> dict:
    ...
    return {
        ...
        "practice_preferences": {
            ...
            "question_count": len(practice_set.get("questions", [])),
            "difficulty_profile": (practice_set.get("questions", [{}])[0] or {}).get("difficulty", "mixed"),
...
def generate_mock_practice_set(workspace: dict, request_payload: dict, *, parent_practice_set_id: str | None = None) -> dict:
    ...
    question_type_cycle = {
        ...
        "mixed": ["multiple_choice", "short_answer", "long_answer"],
    }.get(generation_mode, ["short_answer", "long_answer"])
    answer_key_enabled = bool(request_payload.get("include_answer_key", True))
    rubric_enabled = bool(request_payload.get("include_rubrics", True))
    duration_by_type = {"multiple_choice": 3, "short_answer": 7, "long_answer": 12}
    questions = []
    for index in range(question_count):
        question_type = question_type_cycle[index % len(question_type_cycle)]
        concept_label = topic_text or material.get("title", "the workspace")
        stem = f"Question {index + 1}: explain one grounded idea from {concept_label}."
        if question_type == "multiple_choice":
            stem = f"Question {index + 1}: which option best matches the grounded lecture idea from {concept_label}?"
        questions.append(
            {
                "question_id": make_id("question"),
                "question_type": question_type,
                "stem": stem,
                "expected_answer": "A strong answer cites the uploaded source and explains the concept in plain language." if answer_key_enabled else "",
                "answer_choices": [
                    f"The lecture definition of {concept_label}",
                    "An unrelated but plausible distractor",
                    "A common misunderstanding from outside the lecture",
                    "A vague answer with no grounding",
                ...
                ] if rubric_enabled else [],
                "scoring_guide_text": "Check that the answer stays anchored in the cited material." if answer_key_enabled else "",
                "citations": [citation],
                "covered_slides": [citation.get("slide_number", 1)],
                "difficulty": difficulty,
                "estimated_minutes": duration_by_type.get(question_type, 6),
            }
        )
    estimated_duration_minutes = sum(question.get("estimated_minutes", 0) for question in questions)
    return {
        ...
        "human_loop_summary": {
            "used_inputs": [
                {"key": "topic_text", "label": "Topic focus", "value": topic_text or "All ready lecture materials"},
                {"key": "generation_mode", "label": "Format", "value": generation_mode},
                {"key": "question_count", "label": "Question count", "value": str(question_count)},
                {"key": "difficulty_profile", "label": "Difficulty", "value": difficulty},
            ],
            "follow_up_actions": [
                "Lock any questions you want to keep before generating a revision.",
                "Regenerate only the redundant or weakly grounded questions instead of replacing the whole set.",
            ],
        },
        "coverage_report": {
            ...
            "notes": "Mock practice generation uses the primary ready material as grounded evidence and keeps coverage notes visible.",
        },
    }
